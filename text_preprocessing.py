...
similarity_matrix = cosine_similarity(student_tfidf, job_tfidf)

matches = []
for i, student in enumerate(student_text['text']):
    best_match_index = similarity_matrix[i].argmax()
    best_match_score = similarity_matrix[i].max()
    matches.append({
        "student_text": student,
        "best_job_text": job_text['text'].iloc[best_match_index],
